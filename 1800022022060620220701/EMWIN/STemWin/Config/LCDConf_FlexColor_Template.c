...
void LCD_X_Config(void)
	...
	if(lcddev.id == 0X5510) //0X5510为4.3寸 800x480的屏
	{
		if(lcddev.dir == 0)   //竖屏  // 2001:3520 6352
			{					          // 9147:3832 6288
			GUI_TOUCH_Calibrate(GUI_COORD_X,0,320,0,319);//480
			GUI_TOUCH_Calibrate(GUI_COORD_Y,0,480,0,479);//800
		  }
	  else //横屏
		  {
...
//lcddev.id == 0X7796  目前用的驱动    7796的驱动          2022.01
	else if(lcddev.id == 0X5310 || lcddev.id == 0X6804|| lcddev.id == 0X7796) //0X5510 0X6804为3.5寸 320x480    7796  20220215
	{
		if(lcddev.dir == 0) //竖屏 	
		{							
			GUI_TOUCH_Calibrate(GUI_COORD_X,0,320,0,319);//27页,最后两个参数需要实际屏幕的大小去修改，电容屏的触摸坐标就是实际的坐标值
			GUI_TOUCH_Calibrate(GUI_COORD_Y,0,480,0,479);//最后两个参数需要实际屏幕的大小去修改，电容屏的触摸坐标就是实际的坐标值
		}
		else //横屏
		{
			GUI_TOUCH_SetOrientation(GUI_SWAP_XY|GUI_MIRROR_Y); 
...
//lcddev.id == 0X7796  目前用的驱动    上面是7796的驱动    2022.01
  ...
  case LCD_X_INITCONTROLLER: 
													{
													 ...
													 //LCD_Init(); //初始化LCD 已经在开始初始化了,所以此处不需要初始化。     2022.01
													 return 0;
													}
	default:
  r = -1;
